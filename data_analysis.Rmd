...
for (i in 1:length(outcomes)) {
  country <- "nigeria"
  model <- tidy(lm(paste(outcomes[i], " ~ as.character(Year)*exposed_15 + 
                       cluster_unique + foreign_aid + as.character(v501) +
                       as.character(v013) + as.character(v025) + 
                       as.character(v149) + deaths + as.character(v190)"), 
                       data = nigeria))
  model <- model %>%
  filter(str_detect(term, "time") == TRUE)
  write.csv(model, file = paste0("Results/", country, "_", outcomes[i], ".csv"))
